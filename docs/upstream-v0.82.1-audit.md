...
| `src/models-store.ts`, `test/models-runtime.test.ts` | Store entries can carry ETags; model errors preserve cause details. | Added `StoredModelsEntry.etag`, propagated cached ETag into `ModelRefreshContext.currentETag`, and added `ModelsError` with cause-preserving description. Tests cover ETag propagation and cause text preservation. |
...
| `src/auth/resolve.ts`, auth/OAuth tests | `ModelsError` messages include formatted cause details. | Implemented Swift `ModelsError` with cause preservation for model-runtime/auth style failures; tests assert cause appears in description. |

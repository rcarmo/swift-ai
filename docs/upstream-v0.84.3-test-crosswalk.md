...
| 47 | `packages/ai/test/github-copilot-oauth.test.ts` | ported | `testUpstream0843CopilotCatalogPolicyAndRetryContract`, `testFinalCopilotPolicyPostRetryFailureContinuation`, Copilot OAuth/concurrency tests | Known/tool-capable/policy filtering, 429 Retry-After retry for catalog GET and policy POST, policy IDs, throttling, and structured concurrency/cancellation behavior. |

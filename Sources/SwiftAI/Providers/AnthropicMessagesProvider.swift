...
public enum AnthropicMessagesProvider {
    ...
    private static func streamRequest(model: Model, context: AIContext, options: StreamOptions?, continuation: AsyncStream<AIEvent>.Continuation) async throws {
        ...
        var state = AnthropicStreamState(model: model, tools: context.tools ?? [], providerThinkingLevel: model.anthropicCompat?.supportsMidConvoEffort == true ? (options?.reasoning?.rawValue ?? "high") : nil)
        var buffer = ""
    ...
    public static func processSSEText(_ text: String, model: Model, tools: [Tool]) -> [AIEvent] {
        var events: [AIEvent] = []
        var state = AnthropicStreamState(model: model, tools: tools, providerThinkingLevel: model.anthropicCompat?.supportsMidConvoEffort == true ? "high" : nil)

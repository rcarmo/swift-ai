...
public enum GoogleGenerativeAIProvider {
    ...
    private static func process(data: String, state: inout GoogleStreamState, yield: (AIEvent) -> Void) {
        ...
        if let finish = candidate.finishReason { state.partial.rawStopReason = finish; state.partial.stopReason = mapFinishReason(finish); if state.partial.content.contains(where: { $0.type == "toolCall" }) { state.partial.stopReason = .toolUse } }
    }
    ...
    private static func finish(state: inout GoogleStreamState, yield: (AIEvent) -> Void) { if !state.started { state.started = true; yield(.start(partial: state.partial)) }; closeCurrent(state: &state, yield: yield); state.partial.timestamp = Int64(Date().timeIntervalSince1970 * 1000); if state.partial.stopReason == nil { state.partial.stopReason = .error; state.partial.errorMessage = "Google stream ended without a stop reason" }; if state.partial.stopReason == .error { if state.partial.errorMessage == nil, let raw = state.partial.rawStopReason { state.partial.errorMessage = "Provider stopped with: \(raw)" }; yield(.error(reason: .error, message: state.partial, error: AIError.provider(state.partial.errorMessage ?? "google stream error"))) } else { yield(.done(reason: state.partial.stopReason ?? .stop, message: state.partial)) } }

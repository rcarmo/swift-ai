# Upstream v0.84.4 whole-corpus test crosswalk

Scope: exact official upstream tag `b79e4cc834970cca69daebffab7df1da7d1e52c4`, `packages/ai/test/*.test.ts`.

Baseline: accepted v0.84.3 crosswalk at `4e58f324fae8ebfa98a3d45181fb248072a2afac`.

This is the bounded v0.84.4 corpus inventory. It covers exactly 137 upstream test files. Changed-test markers: `M` = modified in the exact v0.84.3..v0.84.4 range, `A` = added in the range. Changed slice: **5M + 1A** (`fireworks-models.test.ts`, `mistral-http-transport.test.ts`, `openai-completions-reasoning-details.test.ts`, `openai-completions-tool-choice.test.ts`, `zai-coding-plan-models.test.ts`, plus added `openrouter-reasoning-options.test.ts`). Open items: `0`.
...
| 44 | M | `packages/ai/test/fireworks-models.test.ts` | ported | `testUpstream0844GeneratedCatalogMetadata`, `testFireworksKimiK26ModelMetadataAndCompat`, Fireworks metadata tests | Fireworks removal and retained Fireworks compatibility are covered by exact catalog comparator plus metadata assertions. |
...
| 89 | M | `packages/ai/test/openai-completions-tool-choice.test.ts` | ported | `testOpenAICompletionsToolChoice` | Explicit `toolChoice: "none"` serializes even without tools; existing required/tool cases remain covered. |
...
| 105 | A | `packages/ai/test/openrouter-reasoning-options.test.ts` | ported | `testUpstream0844OpenRouterReasoningOptionSemanticsAndCatalog` | OpenRouter supported_efforts maps to optional/mandatory/off thinkingLevelMap semantics and payload omission/none/effort behavior. |
| 106 |  | `packages/ai/test/overflow.test.ts` | ported | overflow tests | Provider overflow classification. |
| 107 |  | `packages/ai/test/pi-messages.test.ts` | ported | PiMessages tests | PiMessages request/SSE/diagnostic handling. |
| 108 |  | `packages/ai/test/provider-error-body-passthrough.test.ts` | ported | provider error body tests | Provider error body passthrough, not catalog metadata. |
| 109 |  | `packages/ai/test/provider-error-body-regression.test.ts` | ported | provider error body regression tests | Provider error body normalization/regression, not catalog metadata. |
| 110 |  | `packages/ai/test/provider-retry.test.ts` | ported | `testProviderRetryPolicyAndDNSClassifier`, `testProviderRetryCapAndCancellation`, `testProviderRetryWiredIntoResponsesTransport` | Provider retry policies/delays/cancellation; not catalog comparator. |
| 111 |  | `packages/ai/test/providers.test.ts` | ported | provider registry/deferred/auth tests | Provider dispatch, auth, deferred capabilities, runtime refresh. |
| 112 |  | `packages/ai/test/qwen-token-plan-models.test.ts` | ported | `testUpstream0811QwenTokenPlanCatalogMetadata`, `testUpstream0841QwenTokenPlanIndividualProvider`, `testUpstream0841QwenTokenPlanIndividualRequestShape` | Qwen Token Plan, CN, and Individual catalog/env/request-shape behavior, including exact Individual seven-model allowlist and qwen `enable_thinking`/`reasoning_effort` payloads. |
| 113 |  | `packages/ai/test/radius-oauth.test.ts` | ported | Radius OAuth/config/runtime tests | Radius OAuth, gateway config, model injection. |
| 114 |  | `packages/ai/test/reasoning-options.test.ts` | adapted | `testReasoningOptionsGeneratorArchitectureEvidence` + exact catalog comparator | Generator policy is adapted to Swift generated `thinkingLevelMap` assertions; no standalone generator helper is shipped. |
| 115 |  | `packages/ai/test/responseid.test.ts` | ported | response ID tests | Response ID propagation across providers. |
| 116 |  | `packages/ai/test/retry.test.ts` | ported | `testUpstream0842UserAgentAndRetryClassifier` and provider retry tests | Buffer-limit retry classifier. |
| 117 |  | `packages/ai/test/sampling-options.test.ts` | ported | sampling options tests | Model/request sampling params merge. |
| 118 |  | `packages/ai/test/stream.test.ts` | ported | Streaming usage preservation tests | Usage updates across streaming terminal/final events retained. |
| 119 |  | `packages/ai/test/supports-xhigh.test.ts` | ported | supportsThinkingLevels metadata tests | DeepSeek V4 Flash low/high and xhigh metadata via exact catalog. |
| 120 |  | `packages/ai/test/telemetry-options.test.ts` | ported | `testTelemetryContextPropagatesThroughStreamDeferredAndImages` | Typed telemetry context propagation. |
| 121 |  | `packages/ai/test/text.test.ts` | ported | text utility tests | Text/sanitize helpers. |
| 122 |  | `packages/ai/test/together-models.test.ts` | ported | Together model metadata tests | Together catalog/reasoning controls. |
| 123 |  | `packages/ai/test/tokens.test.ts` | adapted/live-remainder | token accounting Swift tests + live-remainder classification | Deterministic generic token behavior is covered; newly added Individual live/provider case is credential-gated and unexecuted. |
| 124 |  | `packages/ai/test/tool-call-id-normalization.test.ts` | ported | tool call ID normalization tests | Tool call ID normalization across providers. |
| 125 |  | `packages/ai/test/tool-call-without-result.test.ts` | adapted/live-remainder | tool-call-without-result tests | Pending/tool-call no-result behavior is covered deterministically; live provider matrix remainder is unexecuted. |
| 126 |  | `packages/ai/test/total-tokens.test.ts` | adapted/live-remainder | total-token usage Swift tests + live-remainder classification | Deterministic generic total-token behavior is covered; provider/live additions remain credential-gated. |
| 127 |  | `packages/ai/test/transform-messages-copilot-openai-to-anthropic.test.ts` | ported | transform messages/Copilot Anthropic tests | Message transform from OpenAI/Copilot to Anthropic payloads. |
| 128 |  | `packages/ai/test/unicode-surrogate.test.ts` | adapted/live-remainder | Unicode/surrogate Swift tests + live-remainder classification | Deterministic generic Unicode behavior is covered; newly added Individual live/provider case is credential-gated and unexecuted. |
| 129 |  | `packages/ai/test/uuid.test.ts` | ported | `CoreUtilityTests.testV0803EstimateClampErrorAndRetryUtilities` | UUIDv7 RFC version/variant layout, monotonic lexical order within fixed millisecond, and sequence overflow timestamp advance. |
| 130 |  | `packages/ai/test/validation.test.ts` | ported | `testUpstream0842StrictSchemaAndNullableNullOmission` + validation tests | Optional non-nullable null omission and schema coercion. |
| 131 |  | `packages/ai/test/xai-oauth.test.ts` | ported | xAI OAuth tests | xAI OAuth device/refresh/verification URI complete. |
| 132 |  | `packages/ai/test/xai-responses.test.ts` | ported | `testGrok45ResponsesCatalogAndActualRequestMatrix`, `testUpstream0843XAI46EncryptedReasoningAndUA` | xAI built-in migration to Responses, encrypted reasoning replay, Grok 4.6/default endpoint/reasoning/UA, and actual transport wire assertions. |
| 133 |  | `packages/ai/test/xhigh.test.ts` | adapted/live-remainder | xhigh support tests | XHigh/max mapping is covered deterministically; upstream live provider matrix remainder is unexecuted. |
| 134 |  | `packages/ai/test/xiaomi-models.test.ts` | ported | Xiaomi model placement tests | Xiaomi catalog placement. |
| 135 |  | `packages/ai/test/xiaomi-token-plan-ams-anthropic-empty-signature-smoke.test.ts` | live-only | N/A live/provider smoke | Smoke test requires live Xiaomi/Anthropic-compatible endpoint; deterministic catalog/signature tests cover portable logic. |
| 136 | M | `packages/ai/test/zai-coding-plan-models.test.ts` | ported | `testUpstream0844GeneratedCatalogMetadata`, exact catalog comparator | ZAI GLM-5.3 generated metadata/cost and coding-plan provider parity covered. |
| 137 |  | `packages/ai/test/zen.test.ts` | adapted/live-remainder | misc provider/catalog tests | Deterministic provider/catalog behavior is covered; upstream live/provider matrix remainder is unexecuted. |

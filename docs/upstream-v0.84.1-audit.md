...
| 5 | `packages/ai/scripts/model-data.ts` | Upstream strict `assertExactModelIds` helper maps to Swift's exact source/comparator audit: `scripts/models.v0.84.1.json` must match `scripts/upstream-models.53fa77c.json`, and the generated embedded registry must match the source snapshot. |
...
| 7 | `packages/ai/src/models.generated.ts` | Regenerated exact text catalog: `1220/1220` provider/id pairs, `39` providers, `9` APIs. Image catalog remains `42/42`, `1` provider, `1` API. |
...
| 13 | `packages/ai/test/context-overflow.test.ts` | Existing context overflow Swift tests cover portable behavior; modified upstream fixture remains represented in deterministic local context/error tests. |
...
| 16 | `packages/ai/test/generate-models-strict.test.ts` | New test ported by exact seven-model Individual allowlist assertions plus exact comparator equality before `ModelsGenerated.swift` is accepted. |
...
| 18 | `packages/ai/test/model-data-validation.test.ts` | Ported through `scripts/audit-parity.py` source/comparator/embedded validation, enum raw-value checks, and exact provider/id representative assertions including Individual models. |
...
| 22 | `packages/ai/test/tokens.test.ts` | Existing token accounting tests retained; no catalog/provider token-counting production delta in this slice. |
...
| 24 | `packages/ai/test/total-tokens.test.ts` | Existing total-token usage tests retained; no new production delta in this slice. |
| 25 | `packages/ai/test/unicode-surrogate.test.ts` | Existing Unicode/surrogate text handling tests retained; no new production delta in this slice. |
...
- `scripts/audit-parity.py` enforces exact text and image catalog parity against v0.84.1 source snapshots.

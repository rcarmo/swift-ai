...
| 33 | `packages/ai/src/models-store.ts` | Runtime store maps to Swift actor-backed `ModelRuntime` and `InMemoryProviderModelsStore`; cache/ETag/fallback tested in `testRadiusRuntimeProviderRefreshUsesConfigAndCacheFallback`. |
...
| 35 | `packages/ai/src/models.ts` | Provider/model publication maps to `AIRegistry` and `ModelRuntime.replaceModels`; registry/runtime tests cover replacement and stale removal. |
...
| 42 | `packages/ai/src/providers/faux.ts` | Faux provider deterministic stream/abort/pending behavior covered by `testFauxProvider...` tests and retry-aborted regression. |
...
| 50 | `packages/ai/src/types.ts` | Types/options covered by Codable shape tests, `samplingParams`, `supportsThinkingTokenBudget`, stop reason, usage, diagnostics, and request option assertions. |
...
| 64 | `packages/ai/test/deferred-tools.test.ts` | Matrix row 10; Swift evidence: OpenAI Completions Kimi deferred tool request-body tests and tool-call-without-result behavior. |
...
| 75 | `packages/ai/test/models-runtime.test.ts` | Matrix row 21; Swift evidence: `ModelRuntime` refresh/cache/replacement tests. |
...
| 94 | `packages/ai/test/telemetry-options.test.ts` | Matrix row 40; Swift structural rationale: upstream telemetry context is JS `pi-telemetry` object threading; Swift public `StreamOptions.metadata`/callbacks carry request metadata and have no telemetry package dependency. |
...
| 10 | `deferred-tools.test.ts` | Kimi/deferred tool declarations and background/deferred operation surfaces. | `testOpenAICompletionsKimiDeferredTools`; JS background/deferred handles have no public Swift API yet, documented as structural. |
...
| 21 | `models-runtime.test.ts` | Runtime model store replacement, cache fallback, stale removal. | `ModelRuntime` tests including Radius runtime refresh/cache fallback. |
...
| 40 | `telemetry-options.test.ts` | Telemetry context threads through JS request surfaces, deferred ops, and images. | Structural Swift rationale: no `pi-telemetry` dependency; `StreamOptions.metadata`, `onPayload`, `onResponse`, and image callbacks are typed request seams. |

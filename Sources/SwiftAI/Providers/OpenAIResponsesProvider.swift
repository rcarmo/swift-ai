...
public enum OpenAIResponsesProvider {
    ...
    private static func applyUsage(_ raw: ResponseUsage?, serviceTier: String?, state: inout ResponsesStreamState) { guard let raw else { return }; let cached = raw.inputTokenDetails?.cachedTokens ?? 0; var u = Usage(); u.input = max(0, (raw.inputTokens ?? 0) - cached); u.output = raw.outputTokens ?? 0; u.reasoning = raw.outputTokenDetails?.reasoningTokens ?? 0; u.cacheRead = cached; u.totalTokens = raw.totalTokens ?? (u.input + u.output + u.cacheRead); AIUtilities.applyCost(model: state.model, usage: &u); applyServiceTierMultiplier(serviceTier, model: state.model, usage: &u); state.partial.usage = u }
...
private struct ResponseCompleted: Decodable { var response: Response?; struct Response: Decodable { var id: String?; var status: String?; var model: String?; var serviceTier: String?; var usage: ResponseUsage?; enum CodingKeys: String, CodingKey { case id, status, model, usage; case serviceTier = "service_tier" } } }
private struct ResponseUsage: Decodable { var inputTokens: Int?; var outputTokens: Int?; var totalTokens: Int?; var inputTokenDetails: InputTokenDetails?; var outputTokenDetails: OutputTokenDetails?; enum CodingKeys: String, CodingKey { case inputTokens = "input_tokens"; case outputTokens = "output_tokens"; case totalTokens = "total_tokens"; case inputTokenDetails = "input_tokens_details"; case outputTokenDetails = "output_tokens_details" }; struct InputTokenDetails: Decodable { var cachedTokens: Int?; enum CodingKeys: String, CodingKey { case cachedTokens = "cached_tokens" } }; struct OutputTokenDetails: Decodable { var reasoningTokens: Int?; enum CodingKeys: String, CodingKey { case reasoningTokens = "reasoning_tokens" } } }

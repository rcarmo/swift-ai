...
public enum GoogleGenerativeAIProvider {
    ...
    private static func finish(state: inout GoogleStreamState, yield: (AIEvent) -> Void) { if !state.started { state.started = true; yield(.start(partial: state.partial)) }; closeCurrent(state: &state, yield: yield); state.partial.timestamp = Int64(Date().timeIntervalSince1970 * 1000); if state.partial.stopReason == nil { state.partial.stopReason = .error; state.partial.errorMessage = "Google stream ended without a stop reason" }; if state.partial.stopReason == .error { yield(.error(reason: .error, message: state.partial, error: AIError.provider(state.partial.errorMessage ?? "google stream error"))) } else { yield(.done(reason: state.partial.stopReason ?? .stop, message: state.partial)) } }
